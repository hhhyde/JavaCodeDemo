...
public class Blowfish {
    ...
    private String encStr(String sPlainText, long lNewCBCIV) {
        ...
        for(byte bPadVal = (byte)(buf.length - (nStrLen << 1)); nPos < buf.length; buf[nPos++] = bPadVal) {
            ;
        }

        Blowfish.BlowfishCBC var9 = this.m_bfish;
            ...
            if (nNumOfBytes < 8) {
                ...
                if (nLen == 0) {
                    ...
                    if (nNumOfBytes < nLen) {
                        ...
                        Blowfish.BlowfishCBC var6 = this.m_bfish;
                        synchronized(this.m_bfish) {
    ...
    private static String byteArrayToUNCString(byte[] data, int nStartPos, int nNumOfBytes) {
        ...
        for(int var5 = 0; nNumOfBytes > 0; nNumOfBytes -= 2) {
            sbuf.setCharAt(var5++, (char)(data[nStartPos] << 8 | data[nStartPos + 1] & 255));
            nStartPos += 2;
        }
    ...
    private static class BlowfishECB {
        ...
        public void cleanUp() {
            ...
            for(nI = 0; nI < 256; ++nI) {
                this.m_sbox1[nI] = this.m_sbox2[nI] = this.m_sbox3[nI] = this.m_sbox4[nI] = 0;
            }
